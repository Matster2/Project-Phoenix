...
public class PlayerController : NetworkBehaviour {
	...
	void Shoot()
	{
		if (Time.time > attactRate + m_LastShot) 
		{
			...
			PlayShootSound ();
			GameObject inst = Instantiate(bullet,shootPoint.position,shootPoint.rotation) as GameObject;
	...
	void DropGrenade()
	{
		if (Time.time > dropRate + m_LastShot) 
		{
			...
			GameObject inst = Instantiate(grenade,shootPoint.position,shootPoint.rotation) as GameObject;
	...
	void Damage(int m_damage)
	{
		...
		CmdBroadcastDamage(m_damage);
	}
		
	void CmdBroadcastDamage(int m_damage)
	{
		if(shield>0)
		{
			shield -= m_damage;
